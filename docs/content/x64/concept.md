...
在第一篇文章中的某些部分可能对初学者来说不太清楚, 这就是为什么我们在本文会对这些术语和概念进行重新解释.

**系统调用**: 在计算中, 系统调用(通常缩写为 syscall)是一种编程方式, 处于用户态的计算机程序向操作系统的内核请求服务. 这可能包括与硬件相关的服务(例如, 访问硬盘驱动器), 创建和执行新进程以及与诸如进程调度之类的集成内核服务进行通信. 系统调用提供了进程与操作系统之间的基本接口.
...
**寄存器**: x64 有 16 个通用寄存器, 其中一些低字节寄存器可以作为 32 位, 16 位或 8 位寄存器独立访问.
...
通常情况下, 不需要显示指定数据类型, 因为可以从操作数上做出合理猜测. 当另一个操作数未暗示本操作数的数据类型时, 需要显示指定数据类型, 例如当我们希望从 0x1000 地址读取 1 byte, 0 扩展至 eax 寄存器中时, 就应当显示指定数据类型.
...
正如我在上面所写, 每个汇编程序都由段组成, 它可以是 data, bss 或者 text.

## 伪指令

伪指令虽然不是真正的 x64 机器指令, 但还是经常在数据段中使用.
...
通常, 编程语言可以更改程序的执行顺序, 如 if 语句, switch-case 语句, goto 等, 汇编语言当然也可以. 指令 cmp 用于在两个值之间执行比较, 它可与条件跳转指令一起用于流程控制.

```text
cmp rax, 50
```

指令 cmp 比较 2 个操作数的值, 但不改变它们, 并且不执行任何操作, 但比较结果会保存在标志寄存器中. 条件跳转指令根据标志寄存器的标志位来判断是否进行跳转操作.
...
Q: 编写一个求 1 + 2 + ... + 100 的程序, 并使用 printf 函数打印出结果.
