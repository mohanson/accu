...
上述代码运行后, 在 `2CMXJX8arHRsiiZadVheRLTd6uhP7DpbaJ9hRiRMSGcF` 地址上创建了一个新的代币. 该账户通常被称作代币的"铸造账户". 函数方法 `spl_create()` 接收两个参数, 分别是:

- `decimals`: 小数位, 例如 decimals=9 时, 1000000000 表示 1 个代币.
- `extension`: 添加额外的代币扩展. 最常用的代币扩展功能是为代币提供元数据.
    - `metadata.name`: 代币名称.
    - `metadata.symbol`: 代币符号, 通常是名称的缩写, 就像 btc 之于 bitcoin 一样.
    - `metadata.uri`: 代币元数据的 json 文件网络地址.
