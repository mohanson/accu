...
系统会生成一个新的密钥对, 并将公钥和私钥保存在一个 json 文件中(通常是 ~/.config/solana/id.json). 您会看到类似以下的输出:
...
Solana 在历史上还存在过一种叫做"我的老天爷格式"的公私钥对格式, 该格式是将公私钥对以 base64 编码而成.

提醒: 以 **base64** 编码而成.

该格式之所以会有如此奇怪的命名, 是因为本书作者在使用某个 solana 钱包时, 数次尝试导入私钥无果, 遂无能狂怒研究其源代码, 发现其采用了这种编码格式, 不由得发出了惊呼:

"我的老天爷!"

幸运的是, 该格式仅在 solana 极早期有过小范围的使用, 在当前时间节点下除非是考古人员否则很难再遇到它.
