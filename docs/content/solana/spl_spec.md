...
使用 token-2022 定义一个 spl token 通常需要用到 3 个不同的账户. 账户名与功能如下表所示.

|      类型      |                                    说明                                     |
| -------------- | --------------------------------------------------------------------------- |
| 程序账户       | token-2022 原生程序, 地址 `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`     |
| 代币铸造账户   | token-2022 铸造账户, 代表某种 token 的定义, 包含总量, 权限, 小数精度等信息  |
| 代币元数据账户 | token-2022 派生账户, 存储该 token 的人类可读信息, 如名字, 符号, 图标 url 等 |
...
基于一些历史原因, spl token 被设计为如此, 如果您有机会深入阅读其代码, 亦会发现它内部有着极重的历史包袱. 我们不得不使用两个账户来存储一个代币的信息. 铸造账户是代币的基础, 用来管理代币供应和权限. 元数据账户是基于铸造账户派生出来的一个 pda 数据账户, 用来附加包装纸, 描述代币的名字, logo 等供人类阅读的信息.
