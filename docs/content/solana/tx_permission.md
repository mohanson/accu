...
只读账户无需状态变更, solana 可以据此并行处理更多交易. 并行执行交易一直是区块链行业的大难题, 因为交易会修改状态, 顺序不同, 将导致执行结果不同. 传统上, 我们使用单核单线程来顺序处理交易.

许多公链试图解决这个问题, 以获得更高的 qps, 例如 ethereum 在 [eip-2930](https://eips.ethereum.org/EIPS/eip-2930) 中就打了一个补丁, 允许用户显示指定该笔交易访问了哪些账户数据, 以便节点可以并行处理互不相关的交易. 不过从作者的视角来看, 这个补丁显然十分的不成功, 在本书编写期间, 这种方式并没有在 ethereum 生态系统里被广泛采用.

相比之下, solana 为账号引入的权限系统, 似乎是一种更为高级和优雅的设计.
