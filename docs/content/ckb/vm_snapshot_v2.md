...
类似的问题并不只出现在 witness 上. 例如 `ckb_load_cell_data` 会把某个 cell 的 data 搬进脚本内存; 若目标 cell 恰好是一个很大的 dep cell, 那么一次 syscall 就可能覆盖掉十几页甚至几十页内存. 对 V1 来说, 这些页都会被当作脚本修改后的脏页完整写进快照; 但对链的语义来说, 它们其实只是对某个已上链 cell data 的临时映射.

再比如脚本读取交易结构化数据的场景. 无论是 input / output 的 lock, type, capacity, 还是 header, script hash, out point 等字段, 最终都会以字节串的形式被 syscall 拷贝到 VM 内存中. 单次拷贝的数据量通常不如 witness 或 cell data 大, 但调用频率可能很高. 如果一次执行里反复读取这些元数据, V1 会持续累积大量"看起来被写过, 实际只是从交易抄进来"的页面, 快照膨胀依然明显.

还有一类更隐蔽但同样重要的场景是程序本身. ELF 加载阶段会把代码段, 只读数据段等内容写入虚拟机内存. 从内存实现看, 这当然也是一次次写页; 可这些字节本质上来自那份确定不变的程序镜像. 如果快照把它们再次逐页保存, 就等于把脚本代码本身又打包了一遍. 对体积较大的程序, 这部分开销同样不可忽略.

因此, Snapshot V2 要解决的问题可以更一般地表述为: 只要某段内存的内容是"从某个稳定外部对象按字节搬运而来", 而不是脚本执行过程中新计算出来的结果, 就不应该简单地按 dirty page 处理. witness 只是最容易观察到的例子, cell data, 交易元数据, 以及 ELF 程序段本质上都属于同一类问题.
