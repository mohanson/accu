...
- 语义视角: `r0 = (r0 + r1 + r2) mod 2^64`, 进位输出在 `r1`
...
- 语义视角: `ra` 写入返回地址, 下一条 PC 按 `pc + imm` 计算
...
语义视角: `r0 = (imm << 12) + lo`, 其中 `lo` 是 `imm` 的低 12 位, 并进行符号扩展至 64 位.
...
- 语义视角: 先计算 `r0 + r1` 的进位, 再计算 `r3 = carry + r4`
...
- 语义视角: 先由 `r1 + r2` 计算进位并回写 `r1`, 再计算 `r3 = r1 + r4`
...
- 语义视角: 先由 `r1 + r2` 计算进位, 再计算 `r3 = carry + r4`
