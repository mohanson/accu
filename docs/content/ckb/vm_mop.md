...
| k256_ecdsa        | 37,225,290   | 33,353,859   | **-10.4%** |
| k256_schnorr      | 17,603,180   | 16,370,957   | **-7.0%**  |
| p256              | 28,359,207   | 24,815,077   | **−12.5%** |
| rsa               | 27,871,846   | 27,321,671   | **−2.0%**  |
| secp256k1_ecdsa   | 10,776,242   | 10,732,561   | **−0.4%**  |
| secp256k1_schnorr | 10,465,140   | 10,308,162   | **-1.5%**  |
| sphincsplus_ref   | 279,791,883  | 327,076,711  | +16.9%     |

几乎所有算法在启用 MOP 后都获得了性能提升, 测试中只有一例算法在启用 MOP 后出现了性能回退. 为了分析这些性能变化, 我们统计了每个算法在启用 MOP 后触发的融合操作码类型和总执行次数, 结果如下:

|       算法        |                                  mop                                  |
| ----------------- | --------------------------------------------------------------------- |
| ed25519           | WIDE_MULU(15876), ADCS(52946), ADD3B(1518)                            |
| k256_ecdsa        | WIDE_MULU(63072), ADCS(56685), SBBS(488), ADD3B(1827), ADD3C(12152)   |
| k256_schnorr      | WIDE_MULU(31389), ADCS(22439), SBBS(4), ADD3B(766), ADD3C(5490)       |
| p256              | WIDE_MULU(87082), ADCS(414371), SBBS(4846), ADD3B(15903), ADD3C(262)  |
| rsa               | WIDE_MULU(191533), ADCS(198216)                                       |
| secp256k1_ecdsa   | WIDE_MUL(146), WIDE_MULU(11877), ADCS(53290), ADD3B(1146), ADD3C(613) |
| secp256k1_schnorr | WIDE_MUL(64), WIDE_MULU(11672), ADCS(51557), ADD3B(1127), ADD3C(589)  |
| sphincsplus_ref   | /                                                                     |
...
CKB-VM 的 MOP 融合本质上是在解码阶段用少量模式匹配开销, 换取执行阶段显著的指令级收益: 当指令序列满足融合条件时, 多条 RISC-V 指令可被压缩为单条内部操作, 并映射到宿主机更高效的原生指令路径. 从 2021 Edition 到 2023 Edition, 融合规则从 10 条扩展到 15 条, 覆盖了大整数加减乘除, 远跳转与常量/地址物化等常见热点模式, 这也是链上密码学算法获得整体加速的核心原因.
