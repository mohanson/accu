...
- Secp256k1 公钥的非压缩表示是 64 byte, 压缩表示是 33 byte, 而 ed25519 公钥只有 32 byte. 更短的长度意味着可以直接使用公钥表示地址, 而不是使用公钥的哈希. 同时它具有一些隐藏的好处:
    ...
    - 在 c/c++/asm 中, 复制 32 长度的 byte 数组和 33 长度的 byte 数组, 其效率可能相差 25%. 这是因为 `memcpy()` 通常使用一个 64 位寄存器来搬运数据, 前者需要搬运 4 次, 后者则是 5 次. 参考 Linux 源码: <https://github.com/torvalds/linux/blob/master/arch/x86/boot/copy.S#L18-L32>
